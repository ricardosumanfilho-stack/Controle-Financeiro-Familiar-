...
CREATE TABLE IF NOT EXISTS app_settings (
  id TEXT PRIMARY KEY,
  category TEXT NOT NULL, -- 'salary', 'emergency', 'house', 'rent', 'cofrinho', 'general'
  value JSONB NOT NULL DEFAULT '{}'::jsonb,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

COMMENT ON TABLE app_settings IS 'Armazena configurações globais da família como salários, regras da reserva, meta da casa e aluguel futuro.';
...
CREATE TABLE IF NOT EXISTS credit_cards (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  person TEXT NOT NULL, -- 'Ricardo' | 'Ellen' | 'Família'
...
COMMENT ON TABLE credit_cards IS 'Cartões de crédito individuais e familiares com dias de fechamento e vencimento.';
...
CREATE TABLE IF NOT EXISTS installment_purchases (
  ...
  person TEXT NOT NULL, -- 'Ricardo' | 'Ellen'
  ...
  purchase_date DATE,
  first_due_date TEXT, -- YYYY-MM
  last_due_date TEXT,  -- YYYY-MM
  category TEXT NOT NULL DEFAULT 'Cartão',
  status TEXT NOT NULL DEFAULT 'ativa', -- 'ativa' | 'antecipada' | 'encerrada'
...
CREATE INDEX IF NOT EXISTS idx_card_subscriptions_card ON card_subscriptions(card_id);
CREATE INDEX IF NOT EXISTS idx_card_subscriptions_person ON card_subscriptions(person);
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  description TEXT NOT NULL,
  amount NUMERIC(12, 2) NOT NULL,
  type TEXT NOT NULL, -- 'receita' | 'despesa' | 'investimento' | 'transferencia' | 'rendimento'
  ...
  payment_method TEXT NOT NULL, -- 'pix' | 'debito' | 'credito' | 'dinheiro' | 'boleto' | etc.
  account_or_pot TEXT,
  notes TEXT,
  card_id TEXT REFERENCES credit_cards(id) ON DELETE SET NULL,
  purchase_date DATE,
  installment_info JSONB, -- { "current": 1, "total": 10, "purchaseId": "..." }
...
CREATE TABLE IF NOT EXISTS grocery_trips (
  ...
  trip_type TEXT DEFAULT 'semanal', -- 'semanal' | 'mensal' | 'extraordinaria'
  week_number INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_grocery_trips_date ON grocery_trips(date);
CREATE INDEX IF NOT EXISTS idx_grocery_trips_person ON grocery_trips(person);
...
CREATE TABLE IF NOT EXISTS grocery_month_plans (
  ...
  mode TEXT NOT NULL DEFAULT 'opcao_a', -- 'opcao_a' | 'opcao_b'
  ...
  ellen_planning_type TEXT NOT NULL DEFAULT 'mensal', -- 'mensal' | 'semanal'
...
CREATE INDEX IF NOT EXISTS idx_shopping_lists_month_key ON shopping_lists(month_key);
...
CREATE TABLE IF NOT EXISTS stock_items (
  ...
  status TEXT NOT NULL DEFAULT 'suficiente', -- 'suficiente' | 'baixo' | 'esgotado'
...
CREATE TABLE IF NOT EXISTS cofrinhos (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  type TEXT NOT NULL, -- 'reserva' | 'casa' | 'manutencao' | 'lazer' | 'aluguel_futuro' | 'reforma' | 'outro'
  objective TEXT,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS cofrinho_movements (
  ...
  date DATE NOT NULL,
  type TEXT NOT NULL, -- 'aporte' | 'retirada' | 'rendimento' | 'transferencia_interna'
  amount NUMERIC(12, 2) NOT NULL,
  person TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_cofrinho_movements_date ON cofrinho_movements(date);
...
CREATE TABLE IF NOT EXISTS renovation_expenses (
  ...
  owner_authorized TEXT NOT NULL DEFAULT 'pendente', -- 'sim' | 'pendente' | 'nao'
...
-- Políticas de Acesso: Permite leitura e escrita para usuários autenticados e cliente anon
DO $$
DECLARE
  tbl text;
BEGIN
